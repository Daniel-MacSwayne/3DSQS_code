...
def Camera_Frame_(xi_min=-1, xi_max=1, yi_min=-1, yi_max=1, v_=np.eye(4), k=1):
    
    o_i = np.zeros(3)
    n_i = [None] * 3
    TR = np.array([xi_max, yi_max, 1])
    TL = np.array([xi_min, yi_max, 1])
    BR = np.array([xi_max, yi_min, 1])
    BL = np.array([xi_min, yi_min, 1])
    
    Frame = np.ones((13, 4, 1))
    Frame[:, :3, 0] = [o_i, TR, TL, o_i, BR, BL, o_i, n_i, TR, BR, n_i, TL, BL]
    Frame[:, :3] *= k
    Frame = (v_ @ Frame)[:, :3, 0]
...
class Video_():
    ...
    def Collate_Frames_(self, Input_Dir, Output_Dir, Type='plt', FPS=30):
        self.Input_Dir = Input_Dir
        self.Output_Dir = Output_Dir
        
        Filenames_ = sorted(glob.glob(Input_Dir + '/*.png'))
        N = len(Filenames_)
        
        # print(N)
        
        # if 'Figure' in Filenames_[0]:
        
        if Type == 'plt':
            Index = np.array([int(Filenames_[i].split(' ')[-1][1:-5]) for i in range(N)])
        elif Type == 'int':
            Index = np.array([int(Filenames_[i].split('/')[-1][:-4]) for i in range(N)])
        
        # Sort = np.argsort(Index)
        # Filenames = [Filenames_[i] for i in Sort]
        
        Filenames = [Input_Dir + '\Figure 2024-07-01 122203 ({}).png'.format(i) for i in range(len(Filenames_))]
        # print(Filenames)
        # sys.exit()

        Frame = cv2.imread(Filenames[0], cv2.IMREAD_GRAYSCALE).shape[::-1]
        fourcc = cv2.VideoWriter_fourcc(*'mp4v')
        Video = cv2.VideoWriter(Output_Dir + '\Video.mp4', fourcc, fps=FPS, frameSize=Frame)
    ...
    def Extract_Frames_(self, Input_File, Output_Dir, Downsample=1):
        ...
        while True:
            ...
            if i % Downsample == 0:
                # Save the frame as an image file (e.g., JPEG or PNG)
...
def Add_Camera_(Frame, I__=None, Name='Camera', Scale=100, Color='red', Features=None, Show=False, Save=False):
    """Creates a Feature with a 3D Surface of any form and projects an image onto the surface.
    Surface mesh must have the same Rectangular Dimensions as the Image. 
    The Image is color-clustered to create a custom linear colorscale.
    This is used to color individual tirangular faces.
    ...
    Inputs:     
    VariabLe            Description                         Type            Units       Default 
    Frame               Camera Frame                        array(13, 3)    m          
    I__                 Image to fill Frame                 array(h, w, 3)              None
    Name                Trace Name                          str                         'Camera'
    Features            List of Plotly Traces               list                        None     
    Show                Show the plot?                      bool                        False
    Save                Save the plot?                      bool                        False
    ...
    Features = Add_Line_(Frame, Name=Name, Color=Color, Size=0.01, Features=Features)
